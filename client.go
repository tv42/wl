...
type Display struct {
	...
	OnDeleteId   func(*DisplayDeleteIdEvent)
}
...
func (p *Display) Dispatch(event *Event) {
	switch event.opcode {
	case 0:
		ev := new(DisplayErrorEvent)
		ev.ObjectId = event.Proxy(p.Connection())
		ev.Code = event.Uint32()
		ev.Message = event.String()
		p.ErrorChan <- ev
	case 1:
		ev := new(DisplayDeleteIdEvent)
		ev.Id = event.Uint32()
		p.DeleteIdChan <- ev
	}
}
...
type Registry struct {
	...
	disposeChan      chan struct{}
	OnGlobal         func(*RegistryGlobalEvent)
	OnGlobalRemove   func(*RegistryGlobalRemoveEvent)
}
...
func (p *Registry) handleEvents() {
loop:
	for {
		select {
		case ev := <-p.GlobalChan:
			if p.OnGlobal != nil {
				p.OnGlobal(ev)
			}
		case ev := <-p.GlobalRemoveChan:
			if p.OnGlobalRemove != nil {
				p.OnGlobalRemove(ev)
			}
		case <-p.disposeChan:
			break loop
...
func (p *Registry) Dispatch(event *Event) {
	switch event.opcode {
	case 0:
		ev := new(RegistryGlobalEvent)
		ev.Name = event.Uint32()
		ev.Interface = event.String()
		ev.Version = event.Uint32()
		p.GlobalChan <- ev
	case 1:
		ev := new(RegistryGlobalRemoveEvent)
		ev.Name = event.Uint32()
		p.GlobalRemoveChan <- ev
	}
}
...
func (p *Shm) handleEvents() {
loop:
	for {
		select {
		case ev := <-p.FormatChan:
			if p.OnFormat != nil {
				p.OnFormat(ev)
			}
		case <-p.disposeChan:
...
type Buffer struct {
	...
	disposeChan chan struct{}
	OnRelease   func(*BufferReleaseEvent)
}
...
func (p *Buffer) handleEvents() {
loop:
	for {
		select {
		case ev := <-p.ReleaseChan:
			if p.OnRelease != nil {
				p.OnRelease(ev)
			}
		case <-p.disposeChan:
...
type DataOffer struct {
	...
	disposeChan       chan struct{}
	OnOffer           func(*DataOfferOfferEvent)
	OnSourceActions   func(*DataOfferSourceActionsEvent)
	OnAction          func(*DataOfferActionEvent)
}
...
loop:
	for {
		...
		case ev := <-p.SourceActionsChan:
			if p.OnSourceActions != nil {
				p.OnSourceActions(ev)
			}
		case ev := <-p.ActionChan:
			if p.OnAction != nil {
				p.OnAction(ev)
			}
		case <-p.disposeChan:
...
func (p *DataOffer) Dispatch(event *Event) {
	switch event.opcode {
	case 0:
		ev := new(DataOfferOfferEvent)
		ev.MimeType = event.String()
		p.OfferChan <- ev
	case 1:
		ev := new(DataOfferSourceActionsEvent)
		ev.SourceActions = event.Uint32()
		p.SourceActionsChan <- ev
	case 2:
		ev := new(DataOfferActionEvent)
...
type DataSource struct {
	...
	disposeChan          chan struct{}
	OnTarget             func(*DataSourceTargetEvent)
	OnSend               func(*DataSourceSendEvent)
	OnCancelled          func(*DataSourceCancelledEvent)
	OnDndDropPerformed   func(*DataSourceDndDropPerformedEvent)
	OnDndFinished        func(*DataSourceDndFinishedEvent)
	OnAction             func(*DataSourceActionEvent)
}
...
func (p *DataSource) handleEvents() {
loop:
	for {
		select {
		case ev := <-p.TargetChan:
			if p.OnTarget != nil {
				p.OnTarget(ev)
			}
		case ev := <-p.SendChan:
		...
		case ev := <-p.CancelledChan:
			if p.OnCancelled != nil {
				p.OnCancelled(ev)
			}
		case ev := <-p.DndDropPerformedChan:
			if p.OnDndDropPerformed != nil {
				p.OnDndDropPerformed(ev)
			}
		case ev := <-p.DndFinishedChan:
			if p.OnDndFinished != nil {
				p.OnDndFinished(ev)
			}
		case ev := <-p.ActionChan:
			if p.OnAction != nil {
...
func (p *DataSource) Dispatch(event *Event) {
	switch event.opcode {
	case 0:
		ev := new(DataSourceTargetEvent)
		ev.MimeType = event.String()
		p.TargetChan <- ev
	case 1:
		ev := new(DataSourceSendEvent)
		ev.MimeType = event.String()
		ev.Fd = event.FD()
		p.SendChan <- ev
	case 2:
		ev := new(DataSourceCancelledEvent)
		p.CancelledChan <- ev
	case 3:
		ev := new(DataSourceDndDropPerformedEvent)
		p.DndDropPerformedChan <- ev
	case 4:
		ev := new(DataSourceDndFinishedEvent)
		p.DndFinishedChan <- ev
	case 5:
		ev := new(DataSourceActionEvent)
		ev.DndAction = event.Uint32()
		p.ActionChan <- ev
	}
}
...
type DataDevice struct {
	...
	disposeChan   chan struct{}
	OnDataOffer   func(*DataDeviceDataOfferEvent)
	OnEnter       func(*DataDeviceEnterEvent)
	OnLeave       func(*DataDeviceLeaveEvent)
	OnMotion      func(*DataDeviceMotionEvent)
	OnDrop        func(*DataDeviceDropEvent)
	OnSelection   func(*DataDeviceSelectionEvent)
}
...
func (p *DataDevice) handleEvents() {
loop:
	for {
		select {
		case ev := <-p.DataOfferChan:
			if p.OnDataOffer != nil {
				p.OnDataOffer(ev)
			}
		case ev := <-p.EnterChan:
			if p.OnEnter != nil {
...
func (p *DataDevice) Dispatch(event *Event) {
	...
	case 2:
		...
		p.LeaveChan <- ev
	case 3:
		ev := new(DataDeviceMotionEvent)
		ev.Time = event.Uint32()
		ev.X = event.Float32()
		ev.Y = event.Float32()
		p.MotionChan <- ev
	case 4:
		ev := new(DataDeviceDropEvent)
		p.DropChan <- ev
	case 5:
...
type ShellSurface struct {
	...
	disposeChan   chan struct{}
	OnPing        func(*ShellSurfacePingEvent)
	OnConfigure   func(*ShellSurfaceConfigureEvent)
	OnPopupDone   func(*ShellSurfacePopupDoneEvent)
}
...
func (p *ShellSurface) handleEvents() {
loop:
	for {
		select {
		case ev := <-p.PingChan:
		...
		case ev := <-p.ConfigureChan:
			if p.OnConfigure != nil {
				p.OnConfigure(ev)
			}
		case ev := <-p.PopupDoneChan:
...
func (p *ShellSurface) Dispatch(event *Event) {
	switch event.opcode {
	case 0:
		ev := new(ShellSurfacePingEvent)
		ev.Serial = event.Uint32()
		p.PingChan <- ev
	case 1:
		ev := new(ShellSurfaceConfigureEvent)
		ev.Edges = event.Uint32()
		ev.Width = event.Int32()
		ev.Height = event.Int32()
		p.ConfigureChan <- ev
	case 2:
		ev := new(ShellSurfacePopupDoneEvent)
		p.PopupDoneChan <- ev
	}
}
...
type Surface struct {
	...
	disposeChan chan struct{}
	OnEnter     func(*SurfaceEnterEvent)
	OnLeave     func(*SurfaceLeaveEvent)
}
...
type Seat struct {
	...
	disposeChan      chan struct{}
	OnCapabilities   func(*SeatCapabilitiesEvent)
	OnName           func(*SeatNameEvent)
}
...
func (p *Seat) handleEvents() {
loop:
	for {
		select {
		case ev := <-p.CapabilitiesChan:
			if p.OnCapabilities != nil {
				p.OnCapabilities(ev)
			}
		case ev := <-p.NameChan:
			if p.OnName != nil {
...
func (p *Seat) Dispatch(event *Event) {
	switch event.opcode {
	case 0:
		ev := new(SeatCapabilitiesEvent)
		ev.Capabilities = event.Uint32()
		p.CapabilitiesChan <- ev
	case 1:
		ev := new(SeatNameEvent)
...
type Pointer struct {
	...
	disposeChan      chan struct{}
	OnEnter          func(*PointerEnterEvent)
	OnLeave          func(*PointerLeaveEvent)
	OnMotion         func(*PointerMotionEvent)
	OnButton         func(*PointerButtonEvent)
	OnAxis           func(*PointerAxisEvent)
	OnFrame          func(*PointerFrameEvent)
	OnAxisSource     func(*PointerAxisSourceEvent)
	OnAxisStop       func(*PointerAxisStopEvent)
	OnAxisDiscrete   func(*PointerAxisDiscreteEvent)
}
...
loop:
	for {
		...
		case ev := <-p.AxisSourceChan:
			if p.OnAxisSource != nil {
				p.OnAxisSource(ev)
			}
		case ev := <-p.AxisStopChan:
			if p.OnAxisStop != nil {
				p.OnAxisStop(ev)
			}
		case ev := <-p.AxisDiscreteChan:
...
func (p *Pointer) Dispatch(event *Event) {
	...
	case 3:
		...
		p.ButtonChan <- ev
	case 4:
		ev := new(PointerAxisEvent)
		ev.Time = event.Uint32()
		ev.Axis = event.Uint32()
		ev.Value = event.Float32()
		p.AxisChan <- ev
	case 5:
		ev := new(PointerFrameEvent)
		p.FrameChan <- ev
	case 6:
		ev := new(PointerAxisSourceEvent)
		ev.AxisSource = event.Uint32()
		p.AxisSourceChan <- ev
	case 7:
		ev := new(PointerAxisStopEvent)
		ev.Time = event.Uint32()
		ev.Axis = event.Uint32()
		p.AxisStopChan <- ev
	case 8:
		ev := new(PointerAxisDiscreteEvent)
		ev.Axis = event.Uint32()
		ev.Discrete = event.Int32()
		p.AxisDiscreteChan <- ev
	}
}
...
type Keyboard struct {
	...
	disposeChan    chan struct{}
	OnKeymap       func(*KeyboardKeymapEvent)
	OnEnter        func(*KeyboardEnterEvent)
	OnLeave        func(*KeyboardLeaveEvent)
	OnKey          func(*KeyboardKeyEvent)
	OnModifiers    func(*KeyboardModifiersEvent)
	OnRepeatInfo   func(*KeyboardRepeatInfoEvent)
}
...
func (p *Keyboard) handleEvents() {
loop:
	for {
		select {
		case ev := <-p.KeymapChan:
			if p.OnKeymap != nil {
				p.OnKeymap(ev)
			}
		case ev := <-p.EnterChan:
		...
		case ev := <-p.ModifiersChan:
			if p.OnModifiers != nil {
				p.OnModifiers(ev)
			}
		case ev := <-p.RepeatInfoChan:
			if p.OnRepeatInfo != nil {
				p.OnRepeatInfo(ev)
			}
		case <-p.disposeChan:
			break loop
...
func (p *Keyboard) Dispatch(event *Event) {
	...
	case 4:
		...
		p.ModifiersChan <- ev
	case 5:
		ev := new(KeyboardRepeatInfoEvent)
		ev.Rate = event.Int32()
...
type Touch struct {
	...
	disposeChan chan struct{}
	OnDown      func(*TouchDownEvent)
	OnUp        func(*TouchUpEvent)
	OnMotion    func(*TouchMotionEvent)
	OnFrame     func(*TouchFrameEvent)
	OnCancel    func(*TouchCancelEvent)
}
...
func (p *Touch) Dispatch(event *Event) {
	...
	case 2:
		...
		p.MotionChan <- ev
	case 3:
		ev := new(TouchFrameEvent)
		p.FrameChan <- ev
	case 4:
		ev := new(TouchCancelEvent)
		p.CancelChan <- ev
	}
}
...
func (p *Output) handleEvents() {
loop:
	for {
		select {
		case ev := <-p.GeometryChan:
			if p.OnGeometry != nil {
				p.OnGeometry(ev)
			}
		case ev := <-p.ModeChan:
...
func (p *Output) Dispatch(event *Event) {
	...
	case 1:
		...
		p.ModeChan <- ev
	case 2:
		ev := new(OutputDoneEvent)
		p.DoneChan <- ev
	case 3:
		ev := new(OutputScaleEvent)
